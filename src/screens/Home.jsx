...
export default function Home() {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-b from-void/35 via-transparent to-void/70" />

      <div className="relative z-[1] flex min-h-0 flex-1 flex-col items-center px-4 pb-4 pt-12">
        <p className="font-display text-[10px] font-bold tracking-[0.35em] text-gold/80">EST. 2023 · BLTRM</p>

        <button
          type="button"
          onClick={() => nav('/wallet')}
          className="mt-8 flex w-[86%] max-w-sm flex-col items-center justify-center rounded-[999px] border border-gold/50 bg-gradient-to-b from-[#f2d37a] to-[#9a7318] px-6 py-4 shadow-gold active:scale-[0.98]"
        >
        ...
        <button
          ...
          onClick={() => nav('/ride')}
          className="ignition relative mt-3 mb-2 flex h-28 w-28 items-center justify-center rounded-full border-4 border-gold bg-[radial-gradient(circle_at_50%_40%,#7CFF3A_0%,#146b22_55%,#041108_100%)] shadow-[0_0_40px_rgba(34,224,106,0.75),0_0_18px_rgba(212,175,55,0.8)] active:scale-[0.96]"
        >
...
function Holo({ k, v, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="rounded-xl border border-bolt/40 bg-void/45 px-2 py-2 text-left shadow-[0_0_16px_rgba(34,224,106,0.25)] backdrop-blur-sm"
    >
      <p className="hud-label text-[8px] text-bolt">{k}</p>
